...
plt_s = sankey(src, dst, weights; size = (1400, 800), node_labels = String.(dg.nodes), edge_color = :black, label_size = 9)
...
plt = plot_graph(dg, dag_positions = true, nlabels = dg.nodes, node_color = node_cols, node_size = node_size)
#Makie.save((@__DIR__)*"/tech_pathway_downstream.jpg", plt)

plt_path = plot_graph_path(dg, "Corn Stover", "Nylon"; nlabels = dg.nodes, node_color = node_cols)
#Makie.save((@__DIR__)*"/tech_pathway_path.jpg", plt_path)
...
plt = plot_graph(dg, dag_positions = true, nlabels = dg.nodes, node_color = node_cols)

plt2 = plot_graph_path(dg, "Corn Stover", "Nylon", nlabels = dg.nodes, node_color = node_cols)
...
#Makie.save((@__DIR__)*"/tech_pathway.jpg", plt)

fg = filter_edges(dg, 0, "Optimal Flow"; fn = Base.isequal)

plt_fg = plot_graph(fg, nlabels = dg.nodes, node_color = node_cols, edge_width = 5)
#Makie.save((@__DIR__)*"/filtered_pathway.jpg", plt_fg)
