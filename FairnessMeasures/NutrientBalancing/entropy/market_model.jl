...
total_utility = sum(values(P_limit))-  sum(values(sup_cap))*prod_Pemit["p2"] # always constant for this case study
N = length(PNODES)
@NLconstraint(m, entropy_obj_cons[n in PNODES], entropy <=  -sum( (t[n]/total_utility )*log(t[n]/total_utility) for n in PNODES))
