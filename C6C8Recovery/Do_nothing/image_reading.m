...
for i = 1:length(lat)
   r(i) = img(round(lat_plot(i)), round(long_plot(i)),1);
   g(i) = img(round(lat_plot(i)), round(long_plot(i)),2);
   b(i) = img(round(lat_plot(i)), round(long_plot(i)),3);
end
...
for i = 1:length(lat)
   h(i) = hsl(round(lat_plot(i)), round(long_plot(i)),1)*360;
   s(i) = hsl(round(lat_plot(i)), round(long_plot(i)),2);
   l(i) = hsl(round(lat_plot(i)), round(long_plot(i)),3)*100;
end
...
%%%%
 ...
 r = textscan(ref, '%s %s %f %f', 'Delimiter',',', 'HeaderLines',1);
 fclose(ref);
...
[pixel_2x, pixel_2y] = tp_map(lat_p2, long_p2); % using the function defined for calculation of pixel co-ordinates
...
[pixel_3x, pixel_3y] = tp_map(lat_p3, long_p3); % using the function defined for calculation of pixel co-ordinates
...
[pixel_1x, pixel_1y] = tp_map(lat_p1, long_p1); % using the function defined for calculation of pixel co-ordinates
...
[pixel_rx, pixel_ry] = tp_map(lat_r, long_r);
