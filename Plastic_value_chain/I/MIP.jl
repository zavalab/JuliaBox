...
node_matrix = CSV.read("plastic_data_thesis//node_matrix.csv", DataFrame)

prod_matrix = CSV.read("plastic_data_thesis//product_matrix.csv", DataFrame)
demand_matrix = CSV.read("plastic_data_thesis//demand_matrix.csv", DataFrame)
supply_matrix = CSV.read("plastic_data_thesis//supply_matrix.csv", DataFrame)
technology_matrix = CSV.read("plastic_data_thesis//technology_matrix.csv", DataFrame)


alpha_matrix = readdlm("plastic_data_thesis//alpha_matrix.csv",',');
site_matrix = CSV.read("plastic_data_thesis//site_matrix.csv", DataFrame)
...
for i in NODES
    for j in NODES
        for k in 2:4
    if supply_matrix[j,5]>=100000 && supply_matrix[i,5]>=100000
    C[i,j,k]=1
    end
end
end
...
sum( distance[i,j]*JuMP.value.(f[i,j,pr]) for i in NODES for j in NODES for pr in PRODS if C[i,j,pr]==1 )
