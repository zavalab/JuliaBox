...
comm = MPI.COMM_WORLD
ncores = MPI.Comm_size(comm)                                         #number of cores
serial = false
if ncores == 1
   serial = true
end
rank = MPI.Comm_rank(comm)                                           #core ID
SPP = round(Int, floor(S/ncores))                                    #number of scenarios per processor
...
#@linkconstraint(graph, m[:CVaR] == m[:VaR] + (1/alpha)*(1/S)*sum(bl[s][:phi] for s in owned))
...
if(MPI.Comm_rank(comm) == 0)
    ...
    for i in 1:length(Vbin)
	    ...
	    println("local_max:  opt",local_max_value,  "  max(mytb) ", maximum(MyTB))
	    println("phi:   ", bl[s].colVal[(getvariable(bl[1], :phi).col)])
