...
node_s = CSV.read("time_test//node_s.csv", DataFrame)
...
for j in 1:ttt*100
    NODES[j]=node_s[j,ttt]
end
NODES=round.(Int, NODES)
...
m_l = Model(with_optimizer(Gurobi.Optimizer,MIPGap=1e-2))
#m_l = Model(with_optimizer(Gurobi.Optimizer))
@variable(m_l, f[i in NODES,j in NODES,PRODS]>= 0);
# demand and supply
...
@constraint(m_l, balance[i in NODES, pr in PRODS],s[i,pr]+p[i,pr]+sum(f[j,i,pr] for j in NODES) ==
...
@constraint(m_l, demand_capacity[i in DEMS;dem_node[i] in NODES ], dem[i] <= dem_cap[i]);

# supply capacity constraints
@constraint(m_l, supply_capacity[i in SUPS;sup_node[i] in NODES ], sup[i] <= sup_cap[i]);
...
@constraint(m_l, transcost == sum(prod_trans[pr]*distance[i,j]*f[i,j,pr] for i in NODES for j in NODES for pr in PRODS));
...
RecordValue[ttt]=JuMP.value.(swf)
