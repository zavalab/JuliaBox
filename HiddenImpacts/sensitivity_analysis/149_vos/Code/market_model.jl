...
node_matrix = readdlm("../Data_Set/node_matrix.csv",',');
prod_matrix = readdlm("../Data_Set/product_matrix.csv",',');
demand_matrix = readdlm("../Data_Set/demand_matrix.csv",',');
supply_matrix = readdlm("../Data_Set/supply_matrix.csv",',');
technology_matrix = readdlm("../Data_Set/technology_matrix.csv",',');
Plimit_matrix = readdlm("../Data_Set/Plimit_matrix.csv",',');

alpha_matrix = readdlm("../Data_Set/alpha_matrix.csv",',');
site_matrix = readdlm("../Data_Set/site_matrix.csv",',');
...
#M = 1e20     # big M
λ = 149000      # penalty coefficient ($/tonne P)
...
for n in NODES
	for pr in PRODS
		JuMP.setlowerbound(f[n,n,pr], 0.0)
		JuMP.setupperbound(f[n,n,pr], 0.0)
	end
end

for n in NODES
	for k in NODES
		JuMP.setlowerbound(f[n,k,"p4"], 0.0)
		JuMP.setupperbound(f[n,k,"p4"], 0.0)
	end
end
