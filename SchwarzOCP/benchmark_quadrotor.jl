...
@time optimize!(schwarz;maxiter=50,tol=0,optional=schwarz->(push!(err_schwarz,get_KKT_error(schwarz.model)),push!(time_schwarz,time())))
time_schwarz.-= start
GC.enable(true); GC.gc()
...
@time optimize!(admm;maxiter=50,tol=0,optional=optional=admm->(push!(err_admm,get_KKT_error(admm.model)),push!(time_admm,time())))
time_admm.-= start
GC.enable(true); GC.gc()
