...
y = pixel_1y';
for j=1:length(x)
arrow([x(1,j), y(1,j)], [x(2,j), y(2,j)], 5, 'EdgeColor','b', 'FaceColor','b', 'BaseAngle', 30, 'Length', 10)
end
...
index_30 = find(strcmp(tech_type, 't30'));

for i = 1:length(index_1)
    scatter(lat_plot(index_1(i)), long_plot(index_1(i)),100,'b', 'filled')
end 
...
for i = 1:length(index_9)
    scatter(lat_plot(index_9(i)), long_plot(index_9(i)),100,'y', 'filled')
end 

for i = 1:length(index_30)
    scatter(lat_plot(index_30(i)), long_plot(index_30(i)),40,'y', 'filled')
end 

h4 = scatter(pixel_rx, pixel_ry, 10, 'r', 'filled');
