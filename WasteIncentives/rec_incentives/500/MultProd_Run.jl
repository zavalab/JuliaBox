...
roi =  (getvalue(prod_revenue) - (getvalue(invcost)/365/20) - (getvalue(opcost)/365) - getvalue(transcost) + getvalue(rec_revenue))*365/(getvalue(invcost))*100
...
println(rp, "Revenue from RECs (\$/day)", ",",  getvalue(rec_revenue))
...
println("Revenue from RECs (\$/day)			: ", getvalue(rec_revenue))
