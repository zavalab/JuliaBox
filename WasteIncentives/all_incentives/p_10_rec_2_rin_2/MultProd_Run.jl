...
roi =  (getvalue(prod_revenue) - (getvalue(invcost)/365/20) - (getvalue(opcost)/365) - getvalue(transcost) + getvalue(p_credit) + getvalue(rec_revenue) + getvalue(rin_revenue))*365/(getvalue(invcost))*100
...
println(rp, "Revenue from RECs (\$/day)", ",",  thoudand_to_dollar*getvalue(rec_revenue))
println(rp, "Revenue from RINs (\$/day)", ",",  thoudand_to_dollar*getvalue(rin_revenue))
...
println("Revenue from RECs (\$/day)			: ", thoudand_to_dollar*getvalue(rec_revenue))
println("Revenue from RINs (\$/day)			: ", thoudand_to_dollar*getvalue(rin_revenue))
