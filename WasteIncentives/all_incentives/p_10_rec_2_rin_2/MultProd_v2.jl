...
# LHS: [Thousand $/day]
# RHS: [Thousand $/day]
@constraint(m, obj_p_cons, 20*365*obj == -20*365*prod_revenue + (20*365*transcost + 20*opcost) + invcost - 20*365*p_credit - 20*365*rec_revenue - 20*365*rin_revenue)
@objective(m, Min, obj)
...
# LHS: [Thoudand $/day]
# RHS: [(thousand kwh = mwh/day)*(usd/mwh)*kW_to_MW = Thousand $/day]
@constraint(m, rec_cons, rec_revenue == prod_recovered[prod_num["Electricity"]]*rec_value*kW_to_MW)
# LHS: [Thoudand $/day]
# RHS: [(thousand gal)*(usd/gal) = Thousand $/day]
