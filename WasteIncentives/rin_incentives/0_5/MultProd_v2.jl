...
m = Model(solver=GurobiSolver(Threads = 2, MIPGap = 0.05))
#, MIPGap = 0.0002
...
# Unit conversions
kW_to_MW = 1e-3
rec_value = 100 # Usd per MWh
rin_value = 0.5 # Usd per gallons of cellulosic biofuel
...
@constraint(m, obj_p_cons, obj == -prod_revenue + (transcost + opcost/365) + invcost/20/365 - rin_revenue)
@objective(m, Min, obj)
...
@constraint(m, rin_cons, rin_revenue == prod_recovered[prod_num["Liq.Biomethane"]]*rin_value)

# Nutrient Tax Credits (S.988 Scenario)
# Assuming Virginia's Value of USD 10.10 / lb P = USD 22.37 /kg P
#p_credit_price = 22.37
#@constraint(m, p_credit_cons, p_credit == p_credit_price*sum(p_recovered[p] for p in PRODS) )
