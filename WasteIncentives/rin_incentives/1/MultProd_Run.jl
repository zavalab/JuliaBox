...
roi =  (getvalue(prod_revenue) - (getvalue(invcost)/365/20) - (getvalue(opcost)/365) - getvalue(transcost) + getvalue(rin_revenue))*365/(getvalue(invcost))*100
...
println(rp, "Revenue from RINs (\$/day)", ",",  getvalue(rin_revenue))
...
println("Revenue from RINs (\$/day)			: ", getvalue(rin_revenue))
