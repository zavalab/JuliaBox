...
@constraint(m, obj_p_cons, obj == -prod_revenue + (transcost + opcost/365) + invcost/20/365)
@objective(m, Min, obj)
...
#Value Analysis
for t in TECHS
	if t == "t17" || t == "t18"		# List technologies that you wish to evaluate
		continue
	else
		@constraint(m, sum(y[t,n] for n in NODES) == 0)
	end
end
...
#@constraint(m, rec_cons, rec == prod_recovered["p2"]*1*kW_to_MW)
# @expression(m, rec[n in NODES], demtot[n,"p8"]*kW_to_MW) # 1 REC = 1MWh of electricity generated
