...
roi =  (getvalue(prod_revenue) - (getvalue(invcost)/365/20) - (getvalue(opcost)) - getvalue(transcost) + getvalue(p_credit))*365/(getvalue(invcost))*100
...
println(rp, "Revenue from P Credit (\$/day)", ",",  getvalue(p_credit))
...
println("Revenue from P Credits (\$/day)			: ", getvalue(p_credit))
