...
Cost = dlmread('Cost_ao0_as0.txt')*Costref;
GHGE = dlmread('GHGE_ao0_as0.txt')*GHGEref;
SW = dlmread('SW_ao0_as0.txt')*SWref;
...
l = legend('UP', 'Minimize $\langle\!\langle \mathbf{F}(x) \rangle\!\rangle_{\alpha_\mathcal{O}=1,\alpha_{\mathcal{S}}=0}$','Location','northeast');
