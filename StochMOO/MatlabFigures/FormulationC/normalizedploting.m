% plot compromise solution of Formulation C (nomalized) + Utopian points
...
Cost = dlmread('Cost_ao0.999_as0.999.txt')*Costref;
GHGE = dlmread('GHGE_ao0.999_as0.999.txt')*GHGEref;
SW = dlmread('SW_ao0.999_as0.999.txt')*SWref;
...
l = legend('UP', 'Minimize $\langle\!\langle \mathbf{F}(x) \rangle\!\rangle_{\alpha_\mathcal{O}=1,\alpha_{\mathcal{S}}=1}$','Location','northeast');
