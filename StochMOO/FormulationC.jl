# solution of stochMOO problem (formulation C)
...
# define auxiliary variables for nested cvar
@variable(CHP, N[1:No]>=0, start=2)
@variable(CHP, 0<=t0[1:No]<=10, start=0)
@variable(CHP, F[1:No], start=2)
@variable(CHP, 0<=z<=10, start=0)   
@constraint(CHP, Ndef[j in 1:No], N[j] >= F[j] - z)   
...
for j in 1:NB           
	   ...
	   @variable(CHPap[j], t[1:No], start=0)
           @constraint(CHP, tdef[j in 1:No], t[j] == t0[j])
	   
           # add second-stage variables and constraints
           @variable(CHPap[j], u[1:No] >= 0, start=2)
           @constraint(CHPap[j], u[1] >= (getvariable(CHPap[j],:TCost)) - t[1] )
           @constraint(CHPap[j], u[2] >= (getvariable(CHPap[j],:GHGT))- t[2] )
           @constraint(CHPap[j], u[3] >= (getvariable(CHPap[j],:SWT))-t[3])   
end
@variable(CHP, ob, start=2)
@constraint(CHP, Fdef[i in 1:No], (1-as)*NS*(F[i]-t0[i]) == sum{getvariable(CHPap[j], :u)[i], j in 1:NB})
@constraint(CHP, No*(1-ao)* (ob - z) == sum{N[j],j in 1:No} )
