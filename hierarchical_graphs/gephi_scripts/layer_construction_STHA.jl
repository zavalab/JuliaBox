    ...
    for (i, node) in enumerate(bus)
        ...
        if HA
            ...
            @objective(node, Min,
                ...
                phi_u * D_shed * dt
            ) # Might need to adjust as to whether this includes both ST and DA
        else
...
function link_ST_over_time(graph, dt = .25)
    ...
    for i in 1:(t_len - 1)
        for j in 1:length(STUC_subgraphs[1][:bus])
            ...
            # conventional set
            #gen_conv_set = findall(x -> (x in gen_DA[:, "Generator Name"]) || (x in gen_ST[:, "Generator Name"]), gen_set)
        ...
        if index <= length(getsubgraphs(DA_subgraph)) # If index is less than the number of time points in DAUC problem
            ...
            for k in 1:num_DA_gen
                ...
                for l in 1:4
                    ...
                    @linkconstraint(graph, - (ST_subgraphs_i[l][:bus][bus_name][:Gen_p][gen_name] + ST_subgraphs_i[l][:bus][bus_name][:Gen_m][gen_name])
                                            + (DA_subgraph_i[:bus][bus_name][:Gen_p][gen_name] + DA_subgraph_i[:bus][bus_name][:Gen_m][gen_name]) <= max_ramp_up / dt
                    )                   # test this line of code instead of the uncommented stuff above; see if it resolves the problem without changing to *dt * 4
                end
        ...
        if i == 1
            if min_up_time >= 4
                for j in 1:15
                    subgraph_index = 17 - j
                    s_range = (j):16
                    @linkconstraint(ST_graphs[1], sum(s_list[s_range]) <= getsubgraphs(ST_graphs[1])[subgraph_index][:bus][bus_name][:x][gen_name])
                end
        ...
        if i == 1
            if min_down_time >= 4
                for j in 1:15
                    subgraph_index = 17 - j
                    z_range = j:16
                    @linkconstraint(ST_graphs[1], sum(z_list[z_range]) <= 1 - getsubgraphs(ST_graphs[1])[subgraph_index][:bus][bus_name][:x][gen_name])
                end
    ...
    for (i, index) in enumerate(tau_range)
        ...
        for k in 1:ST_len
            ...
            if (tau != 1)
                ...
                @linkconstraint(HA_subgraph, bus_last[:Gen_p][gen_name] + bus_last[:Gen_m][gen_name] -
                    bus_next[:Gen_p][gen_name] - bus_next[:Gen_m][gen_name] <=
