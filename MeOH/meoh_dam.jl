...
#for laptop format
DAM_matrix = CSV.read("/MeOH//DAM_pan_price.csv")
Interval=8784;
T=1:Interval;
...
Coef_BOP_cost=258000*54 # BOP investment is 258000/MW, making one ton h2 uses 54mwh of power, one hour contains 12 5mins
...
D_H2=11 #Demand of H2: 11/12t / 5mins
UB_grid=4.62*4#
...
Record_fe_day_l_DAM_Pan=zeros(366,24)
Record_fr_day_l_DAM_Pan=zeros(366,24)
Record_fs_day_l_DAM_Pan=zeros(366,24)
Record_storage_day_l_DAM_Pan=zeros(366,24)


for i in 1:366
for j in 1:24
k=(i-1)*24+j
Record_fe_day_l_DAM_Pan[i,j]=Record_fe[k]
Record_fr_day_l_DAM_Pan[i,j]=Record_fr[k]
Record_fs_day_l_DAM_Pan[i,j]=Record_fs[k]
Record_storage_day_l_DAM_Pan[i,j]=Record_s[k]
end
end


Record_fe_day_DAM_Pan=zeros(366)
Record_fr_day_DAM_Pan=zeros(366)
Record_fs_day_DAM_Pan=zeros(366)
Record_storage_day_DAM_Pan=zeros(366)


for i in 1:366
Record_fe_day_DAM_Pan[i]=sum(Record_fe_day_l_DAM_Pan[i,j] for j in 1:24)
Record_fr_day_DAM_Pan[i]=sum(Record_fr_day_l_DAM_Pan[i,j] for j in 1:24)
Record_fs_day_DAM_Pan[i]=sum(Record_fs_day_l_DAM_Pan[i,j] for j in 1:24)
Record_storage_day_DAM_Pan[i]=sum(Record_storage_day_l_DAM_Pan[i,j] for j in 1:24)
end
