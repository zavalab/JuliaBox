...
open("3d_cont_data2.json", "r") do f
    global cont_data
    dicttxt = readstring(f)  # file information to string
...
bin_actives = [bin_data[string(i)]["y"] .>= 1e-8 for i = 1:length(bin_data)]
...
grid()
# plt[:savefig]("3d_SF_cost_bin.png", pad_inches = 0, dpi = 300, transparent = false)
...
grid()
# plt[:savefig]("3d_SF_cost_overlay.png", pad_inches = 0, dpi = 300, transparent = false)
...
table[2:end, 7] = differences / 1000 * 100
writecsv("3d_results.csv", table)
