...
open("../Summary_Output/summary_results.csv", "a") do rp
...
for pp in PRODS
    open("../Flow_Output/flow_results_"*"$(pp)"*".csv", "w") do ff
        print(ff,",")
...
open("../Summary_Output/clearing_prices.csv", "w") do cp
    print(cp,",")
...
open("../Summary_Output/market_player.csv","w") do mp # We can divide this to several files when there are too many players
...
open("../Summary_Output/supply_results.csv","w") do sp
...
open("../Summary_Output/demand_results.csv","w") do dp
...
open("../Summary_Output/P_results.csv","w") do PP
