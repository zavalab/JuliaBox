...
function OptimalControl(u0, t_start, soc_min, soc_max)
    ...
    expectedrevenue = 409723*P_nominal/1000	   # 241407
    ...
    Un_guess = 9.99877 -9.99961*theta_n_guess.^0.5 -9.98836*theta_n_guess + 8.2024*theta_n_guess.^1.5  + 0.23584./theta_n_guess -2.03569*theta_n_guess.^(-0.5) - 1.47266*exp.(-1.14872*theta_n_guess+2.13185)-
         9.9989*tanh.(0.60345*theta_n_guess-1.58171)
    Up_guess = 7.49983 - 13.7758*theta_p_guess.^0.5 + 21.7683*theta_p_guess - 12.6985*theta_p_guess.^1.5 + 0.0174967./theta_p_guess -0.41649*theta_p_guess.^(-0.5) - 0.0161404*exp.(100*theta_p_guess-97.1069) +
    ...
    if soc_min != soc_max
        @variable(m, capacity_remain*(0.01+soc_min_stop)*csnmax<=csn_avg[t in 2:Nt]<=capacity_remain*(soc_max_stop-0.01)*csnmax,start = csn_avg0)  # be a little more conservative
        JuMP.setlowerbound(csn_avg[Nt], capacity_remain*soc_min*csnmax)
    	JuMP.setupperbound(csn_avg[Nt], capacity_remain*soc_max*csnmax)
    else
	@variable(m, capacity_remain*(0.01+soc_min_stop)*csnmax<=csn_avg[t in 2:Nt-1]<=capacity_remain*(soc_max_stop-0.01)*csnmax,start = csn_avg0)
    end
