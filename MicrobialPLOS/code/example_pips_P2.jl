...
args = Dict(
    ...
    :maxr=>Inf,                    # Maximum absolute value of parameters
...
args[:y_start]=Dict((exp,i,ii,j,k)=>.0
...
args[:r_scale]=8.5e-4
args[:y_scale]=Dict(exp=>[Dict((ii,j,k)=>max(1e-1,data[exp][i][ii][:y][j][round(Int64,(k-1)/args[:n_disc][exp])+1])
...
t0=time_ns()
MPI.Init()
outs[:status]=Plasmo.pipsnlp_solve(outs[:graph],outs[:node_parent],outs[:node_children])
MPI.Finalize()
