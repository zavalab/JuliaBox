...
args = Dict(
    ...
    :n_total_species=>24,       # Total number of species
...
args[:r_scale]=1e-3
args[:y_scale]=Dict(exp=>[Dict((ii,j,k)=>max(1e-2,data[exp][i][ii][:y][j][floor(Int64,(k-1)/args[:n_disc][exp])+1])
