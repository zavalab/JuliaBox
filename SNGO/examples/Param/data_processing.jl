...
# Define type ExpData
type ExpData
  nameExp::String # Name of experiment
...
if includeMono==true
  ...
  for i=1:numDataMono0
    ...
    push!(data,SetExpData("mono",speciesMono0[i],time0,
      abund0,ICmono0[i],numOfDiscMono,1/length(time0)))
...
if includePair==true
  ...
  for i=1:numDataPair
    ...
    push!(data,SetExpData("Pair",speciesPair[i],t0,
      copy(absAbundance0),ICpair[i,:],numOfDiscMulti,1/l))
    ind0=length(data)
    push!(dataIC,ind0)

    push!(data,SetExpData("Pair",speciesPair[i],t1,
      copy(absAbundance1),"noIC",numOfDiscMulti,1/l))
    ind1=length(data)
    push!(dataNoIC,ind1)

    push!(data,SetExpData("Pair",speciesPair[i],t2,
      copy(absAbundance2),"noIC",numOfDiscMulti,1/l))
    ind2=length(data)
    push!(dataNoIC,ind2)
...
if includePW22==true
  ...
  for i=1:numDataPW22
    ...
    push!(data,SetExpData("PW22",speciesPW22[i],t0,
      copy(absAbundance0),ICPW22[i,:],numOfDiscMulti,1/l))
    ind0=length(data)
    push!(dataIC,ind0)
...
if includeTM1==true
  ...
  for i=1:numDataTM1
    ...
    push!(data,SetExpData("TM1",speciesTM1[i],t0,
      copy(absAbundance0),ICTM1[i,:],numOfDiscMulti,1/l))
    ind0=length(data)
    push!(dataIC,ind0)
...
if includeL1O6==true
  ...
  for i=1:numDataL1O6
    ...
    push!(data,SetExpData("L1O6",speciesL1O6[i],t0,
      copy(absAbundance0),ICL1O6[i],numOfDiscMulti,1/l))
    ind0=length(data)
    push!(dataIC,ind0)
