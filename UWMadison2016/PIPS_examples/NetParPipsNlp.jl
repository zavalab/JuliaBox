...
ModelData() = ModelData(nothing,0,0,0,0,0,Int[],Int[], Float64[], Int[], Int[], Float64[],Int[],Int[],Float64[], Int[], Int[], Float64[], 0, 0, Float64[],Float64[],Float64[],Float64[], Int[], Int[],nothing, nothing, nothing, nothing)
	...
	function str_eval_g(nodeid,x0,x1,new_eq_g, new_inq_g)
	 ...
	 function str_eval_h(rowid,colid,x0,x1,obj_factor,lambda,mode,rowidx,colptr,values)
		...
		if rowid==colid 
		   ...
		   node_val = ones(Float64,length(node_Hrows))
                   local_scl = (node.objSense == :Min) ? 1.0 : -1.0
                   eval_hesslag(local_data.d, local_unsym_values, local_x, obj_factor*local_scl, lambda)
