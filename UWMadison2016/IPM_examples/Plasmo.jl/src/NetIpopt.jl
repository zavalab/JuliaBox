...
function Ipopt_solve(master::JuMP.Model)
	 ...
	 net_rowLb = Float64[]
	     ...
	     if idx <=numLeaf
	        local_data.d = JuMP.JuMPNLPEvaluator(node)
         	...
         	local_numconnectRows = length(node.linconstr)
         	for c in 1:local_numconnectRows
             	    coeffs = node.linconstr[c].terms.coeffs
             	    local_data.jacnnz += length(coeffs)
         	end
		net_jacnnz += local_data.jacnnz	     
             end
         end
	     ...
	     local_numconnectRows = length(node.linconstr)
             for c in 1:local_numconnectRows
             	 ...
             	 for (it,ind) in enumerate(coeffs)
                     push!(Icon, numconnectRows + c)
                     push!(Jcon, getData(vars[it].m).relative_n + vars[it].col)
                     push!(Vcon, ind)
             	 end
             end
	     numconnectRows += local_numconnectRows
	 end
	 Mcon = sparse(Icon, Jcon, Vcon, numconnectRows, net_n)
         ...
         function eval_f_cb(x)
                ...
                n_start = 1
                n_end = 0
                for (idx,node) in enumerate(leafModelList)
                    local_data = getData(node)
                    n_end += local_data.n
