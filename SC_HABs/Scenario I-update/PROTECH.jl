...
v = Dict(zip(algae, algae_matrix[:,3])); # cell volume of each algae [μm3]
...
P = Dict(zip([0;day], [120; zeros(length(day),1)]));  # concentration of P in lake [mg/m3] with initial value (at end of day, or available the next day)
N = Dict(zip([0;day], [960;zeros(length(day),1)]));  # concentration of N in lake [mg/m3] with initial value
    ...
    for l in layer
        for a in algae
            ...
            if z[l] <= hp[a,l]
                ...
                rθI[(a,l)] = 0.223*αr[a]*I0*3600*24*exp(-ε[d]*z[l]); #shoudl be 0.223*3600*24
            end
