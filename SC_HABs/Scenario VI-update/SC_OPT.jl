...
@constraint(m, [n in NODES[271:272], t in TECHS[1:1]], y[n,t] == 1);
@constraint(m, [n in NODES[271:272], t in TECHS[2:end]], y[n,t] == 0);
@constraint(m, [n in NODES[273:end], t in TECHS[2:3]], y[n,t] == 1);
@constraint(m, [n in NODES[273:end], t in [TECHS[1];TECHS[4:end]]], y[n,t] == 0); 
...
@constraint(m, [pr in PRODS, n in NODES], I[n,pr,0] == 0.49*sum(inv_cap[inv]*(inv_prod[inv] == pr)*(inv_node[inv] == n) for inv in INVS));  # set initial values of inventory levels
...
@constraint(m, sumTP == sum(TP[n,τ] for n in NODES for τ in TIME) - sum(TP[NODES[270], t] for t in TIME));
@constraint(m, sumTN == sum(TN[n,τ] for n in NODES for τ in TIME) - sum(TN[NODES[270], t] for t in TIME));
...
λ1 = 1;
λ2 = 1e-5*33.07*1000; # apoorva $/tonne
λ3 = 1e-5*4.54*1000;  # traci equivalent
