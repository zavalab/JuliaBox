#I could easily use the LightGraphs interface.  Just make my own node and edge types and have them
#index to indices within LightGraphs.
...
type PlasmoGraph <: AbstractPlasmoGraph
    ...
    subgraphlist::Vector{AbstractPlasmoGraph}  #How Plasmo manages variable couplings
...
#PlasmoGraph Constructors
PlasmoGraph() = PlasmoGraph(DiGraph(),gensym(),0,AbstractGraph[],Dict(),Dict{Int,AbstractNode}(),Dict{LightGraphs.Edge,AbstractEdge}())
function PlasmoGraph(g::AbstractGraph)
    pgraph = PlasmoGraph(g,gensym(),0,AbstractGraph[],Dict(),Dict{Int,AbstractNode}(),Dict{Lightgraphs.Edge,AbstractEdge}())
...
#nodes and edges are integers and pairs.  When adding nodes, it creates my node data structure and also adds the vertex to the underlying graph
...
# Node constructors
PlasmoNode() = PlasmoNode(Dict{PlasmoGraph,Int}(), Symbol("node"),Dict{Any,Any}(:model => nothing,:couplingfunctions => Dict{AbstractGraph,Vector{Function}}(),:couplingarguments => Dict{AbstractGraph,Dict{Function,Any}}()))
function PlasmoNode(g::PlasmoGraph)
    add_vertex!(g.graph)
    i = nv(g.graph)
    label = Symbol("node"*string(i))
    node = PlasmoNode(Dict(g => i),label,Dict(:model => nothing,:couplingfunctions => Dict{AbstractGraph,Vector{Function}}(),:couplingarguments => Dict{AbstractGraph,Dict{Function,Any}}()))
    g.nodes[i] = node
    return node
end
...
#Edge constructors
PlasmoEdge() = PlasmoEdge(Dict{PlasmoGraph,LightGraphs.Edge}(), Symbol("edge"),Dict{Any,Any}(:model => nothing,:couplingfunctions => Dict{AbstractGraph,Vector{Function}}(),:couplingarguments => Dict{AbstractGraph,Dict{Function,Any}}()))
function PlasmoEdge(g::PlasmoGraph,edge::LightGraphs.Edge)
    add_edge!(g.graph,edge)
    label = Symbol("edge"*string(edge))
    pedge = PlasmoEdge(Dict(g => edge),label,Dict(:model => nothing,:couplingfunctions => Dict{AbstractGraph,Vector{Function}}(),:couplingarguments => Dict{AbstractGraph,Dict{Function,Any}}()))
    g.edges[edge] = pedge
    return pedge
end
