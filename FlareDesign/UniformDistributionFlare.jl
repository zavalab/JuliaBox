...
@variable(FlareDesign, MaxK>=0, start=2100)	#Average value of K in different scenarios
@variable(FlareDesign, MinK>=1000, start=1676)		#Standar deviation of K


@NLconstraint(FlareDesign, constr12[j=1:s], MaxK>=K[j])
@NLconstraint(FlareDesign, constr13[j=1:s], MinK<=K[j])

@NLconstraint(FlareDesign, constr14[j=1:s], K[j]-MinK<=MaxK-MinK)
@NLconstraint(FlareDesign, constr15[j=1:s], (K[j]-MinK)>=0)


@NLconstraint(FlareDesign, 0.98*(MaxK-MinK)+MinK<=2000) #Restriction

#Objective FUNCTION
@NLobjective(FlareDesign,Min, Mcost-MinK*10)
...
RadMin=minimum(Rad)
RadMax=getvalue(MaxK)
CDFRad=zeros(1000)

for n=1:s
CDFRad[n]=(Rad[n]-RadMin)/(RadMax-RadMin)
end
xCDF=sortperm(Rad)
