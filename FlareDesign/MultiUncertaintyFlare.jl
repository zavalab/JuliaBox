...
Heatc=21500	#Heat of combustion (Btu/lb)
p2=14.7		#Absolute pressure (psi)
wuinf=29.3	#Wind velocity (ft/s)
...
Hc=Heatc*0.15*randn(1000)+Heatc	
#uinf=wuinf*0.15*randn(1000)+wuinf	

#UNIFORM DISTRIBUTION
#qm=rand(1000)*(1.15*mf-0.85*mf)+0.85*mf
#Hc=rand(1000)*(1.15*Heatc-0.85*Heatc)+0.85*Heatc
uinf=rand(1000)*(1.15*wuinf-0.85*wuinf)+0.85*wuinf

#GUMBLED DISTRIBUTION
qm=mf*0.15*(-log.(-log.(rand(1000))))+mf
#Hc=Heatc*0.15*(-log.(-log.(rand(1000))))+Heatc
#uinf=wuinf*0.15*(-log.(-log.(rand(1000))))+wuinf
...
@NLconstraint(FlareDesign, constr3[j=1:s], log(delx[j])==log(L[j]*0.9838)+0.0754*(log(uinf[j])-log(uj[j])))

@NLconstraint(FlareDesign, constr4[j=1:s], log(dely[j])==log(L[j]*0.0985)-0.705*(log(uinf[j])-log(uj[j])))
...
savefig("/home/javier/Documentos/FiguresPaperDistribution/zMU_Rad_95.jpg")
...
ylabel("Y",fontsize=14)
savefig("/home/javier/Documentos/FiguresPaperDistribution/zMU_flow_95.jpg.jpg")

nbins = 75 			#Number of bins
figure("Heat of combustion",figsize=(10,10))					
k=plt[:hist](Hc,nbins,normed="True") 	#Histogram
grid("on")
xlabel(L"$\ Heat \ of \ combustion, \ H = \frac{BTU}{lb}$",fontsize=14)
ylabel("Y",fontsize=14)
savefig("/home/javier/Documentos/FiguresPaperDistribution/zMU_Heat_95.jpg.jpg")

nbins = 75 			#Number of bins
figure("Wind speed",figsize=(10,10))					
k=plt[:hist](uinf,nbins,normed="True") 	#Histogram
grid("on")
xlabel(L"$\ Wind \ speed, \ u_inf = \frac{ft}{s}$",fontsize=14)
ylabel("Y",fontsize=14)
savefig("/home/javier/Documentos/FiguresPaperDistribution/zMU_Wind_95.jpg.jpg")
