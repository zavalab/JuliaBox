...
@variable(FlareDesign, MeanK, start=900)			#Mean of K in all scenarios
...
#Desired probability
#prob=0.5		#probability of 50%
#prob=0.8		#probability of 80%
#prob=0.9		#probability of 90%
prob=0.95 		#probability of 95%
#prob=0.98		#probability of 98%
#prob=0.999		#probability of 99.9%

@NLconstraint(FlareDesign, MeanK==sum(K[i]*(1/s) for i=1:s))

@NLconstraint(FlareDesign, MeanK<=kmax/-log(1-prob)) 
...
for n=1:s
CDFRad1[n]=1-exp(-Rad[n]/getvalue(MeanK))
end
xCDF1=sortperm(Rad)
...
for n=1:s
PDFRad[n]=(1/getvalue(MeanK))*exp(-Rad[n]/getvalue(MeanK))
end
xPDF=sortperm(Rad)
...
for n=1:s
PDFflow[n]=(1/mean(qm))*exp(-qm[n]/mean(qm))
end
...
savefig("/home/javier/Documentos/FiguresPaperDistribution/ED_His_Rad_MM_95.pdf")
...
setp(ax[:get_yticklabels](),fontsize=16)
setp(ax[:get_xticklabels](),fontsize=16)
savefig("/home/javier/Documentos/FiguresPaperDistribution/ED_CDF_MM_95.pdf")
